...
// API routes — standalone handler that bypasses TanStack Start API bundling
//
// Reads credentials from the environment (set PRISM_DB_URL + PRISM_DB_AUTH_TOKEN
// or rely on TURSO_DATABASE_URL + TURSO_AUTH_TOKEN + OPENAI_API_KEY).
const { handleApiRequest } = await import("./src/api-handler.ts");
  ...
  try {
    Bun.serve({
      ...
      async fetch(req) {
        ...
        // API routes (handled directly, bypassing TanStack Start bundling)
